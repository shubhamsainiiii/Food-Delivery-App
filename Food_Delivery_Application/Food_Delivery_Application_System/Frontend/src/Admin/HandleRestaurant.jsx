...
//                         className={`px-4 py-1 rounded-full border text-sm font-medium ${selectedStatus === status ? 'bg-yellow-100 text-yellow-700 border-yellow-400' : 'text-gray-600 border-gray-300'}`}
//                     >
        ...
        <div className="flex min-h-screen">
            ...
            <div className="flex-1 p-6">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                    {statusMap[selectedStatus].icon}
                    <span className={statusMap[selectedStatus].color}>
                        {statusMap[selectedStatus].label} Restaurant Requests
                    </span>
                </h2>
                ...
                <div className="flex gap-4 mb-4 flex-wrap">
                    {Object.keys(statusMap).map(status => (
                        <button
                            key={status}
                            onClick={() => setSelectedStatus(status)}
                            className={`px-4 py-1 rounded-full border text-sm font-medium ${selectedStatus === status
