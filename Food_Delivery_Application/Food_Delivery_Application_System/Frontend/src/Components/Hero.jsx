...
//                         />
//                         <button className="bg-gradient-to-r from-orange-500 to-orange-400 px-6 py-2 text-white font-medium">
//                             Search
//                         </button>
//                     </div>
//                 </div>
...
//                                 <div className="flex flex-col items-center justify-center min-w-[80px] p-2 bg-gray-100 rounded-md hover:scale-105 transition-transform">
//                                     <img
//                                         src={item.image}
//                                         alt={item.name}
...
//         // Debounce for better UX (wait user stop typing)
//         if (timeoutRef.current) clearTimeout(timeoutRef.current);

//         timeoutRef.current = setTimeout(async () => {
//             try {
//                 const res = await axios.get(`/api/food/search?query=${encodeURIComponent(searchTerm)}`);
...
//     // Optional: navigate to food detail or show modal
//     const handleSelect = food => {
//         alert(`Selected food: ${food.foodName}`);
...
//                     </h1>
//                     <div className="flex bg-white rounded-full overflow-hidden w-full max-w-xl relative">
//                         <div className="flex items-center px-4 text-orange-600">
//                             <FaSearch />
//                         </div>
//                         <input
...
//                         {showResults && (
//                             <div className="absolute top-full left-0 right-0 bg-white text-black rounded-b-md shadow-lg max-h-64 overflow-y-auto z-20">
...
//                                     <div
//                                         key={item._id}
//                                         className="px-4 py-2 cursor-pointer hover:bg-orange-200 flex items-center gap-3"
//                                         onMouseDown={() => handleSelect(item)}
//                                     >
//                                         {/* Optionally display food image if available */}
//                                         <span className="font-semibold">{item.foodName}</span>
//                                         <span className="text-xs text-gray-500">
//                                             {item.category} {item.restaurantId && `| ${item.restaurantId.restaurantName}`}
//                                         </span>
//                                     </div>
//                                 ))}
//                             </div>
                ...
                <div className="max-w-6xl mx-auto px-4">
                    ...
                    >
                        {categories.map((item, index) => (
                            <SwiperSlide key={index}>
                                <div
                                    className="flex flex-col items-center justify-center min-w-[80px] p-2 bg-gray-100 rounded-md hover:scale-105 transition-transform cursor-pointer"
                                    onClick={() => alert(`Category clicked: ${item.name}`)}
                                >
                                    <img src={item.image} alt={item.name} className="h-16 w-16 object-contain mb-2" />
                                    <p className="text-black font-medium text-center text-sm">{item.name}</p>
                                </div>
                            </SwiperSlide>
                        ))}
